...
function Fisher(element, dimensions, background) {
  ...
  // Extended Functionality with random initializer of the cubeSize and spacing,random color, using Fragment Shader MeshFaceMaterial()
  var colours = [0xC41E3A+randomInt(-1024, 1024), 0x009E60+randomInt(-1024, 1024), 0x0051BA+randomInt(-1024, 1024), 0xFF5800+randomInt(-1024, 1024), 
	  0xFFD500+randomInt(-1024, 1024), 0xFFFFFF+randomInt(-1024, 0)];
   
  var faceMaterials = colours.map(function(c) {
        return new THREE.MeshPhongMaterial({ ambient: c });
      }),
      cubeMaterials = new THREE.MeshFaceMaterial(faceMaterials);
  var cubeSize = randomInt(3, 5),
      spacing = randomInt(1, 5) / 10;
